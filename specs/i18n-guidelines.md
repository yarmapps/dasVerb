...
## 4. Добавление новых ключей переводов (Только EN и RU по умолчанию)

* По умолчанию переводы создаются **СТРОГО ТОЛЬКО для двух языков**:
  1. **Английский:** `src/translations/en.json` (базовый язык системы).
  2. **Русский:** `src/translations/ru.json`.
* Переводы на остальные 9 поддерживаемых языков (`es`, `fr`, `it`, `pl`, `pt`, `tr`, `uk`, `ar`, `fa`) добавляются **ИСКЛЮЧИТЕЛЬНО по прямому и явному запросу пользователя**.
* Для остальных языков при отсутствии перевода работает **автоматический Fallback на английский язык (`en`)** в `intlService`.

### Порядок добавления нового ключа:
1. Добавить поле в TypeScript-интерфейс `Translation` в [`src/types/intl.ts`](../src/types/intl.ts).
2. Добавить ключ и текст перевода в `src/translations/en.json` и `src/translations/ru.json`.
3. Запустить `yarn test`, чтобы убедиться в прохождении тестов.
