...
type PrepositionsPracticeRouteProp = RouteProp<PracticeStackParamList, 'PrepositionsPracticeList'>;

const CEFR_TABS = ['A1', 'A2', 'B1', 'B2'] as const;
type CefrTab = (typeof CEFR_TABS)[number];

export function PrepositionsPracticeListScreen(): React.JSX.Element {
  const intl = useIntl();
  ...
  const [levels, setLevels] = useState<PrepositionLevelData[]>([]);
  ...
  const loadData = useCallback(
    async (targetCefr?: CefrTab) => {
      try {
        ...
        const loadedLevels = await verbDataService.getPrepositionLevelsByCefr(levelToLoad);
        setLevels(loadedLevels);

        const progressMap: Record<string, VerbProgress> = {};
        loadedLevels.forEach(lvl => {
          progressMap[lvl.id] = progressService.getPrepositionLevelProgress(lvl.id);
        });
        setLevelProgressMap(progressMap);
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error('[PrepositionsPracticeListScreen] Failed to load preposition levels:', error);
      }
    },
    [activeCefr],
  );
  ...
  return (
    <ScreenBackground>
      ...
      <ScreenHeader
        title={intl.formatMessage({ id: 'prepositionsPracticeListScreen.title' })}
        showBackButton={true}
      ...
      >
        {levels.map((lvl, _index) => {
              ...
              >
                <PracticeCheckpointCard
                  ...
                  subtitle={intl.formatMessage({
                    id: 'prepositionsPracticeListScreen.checkpointSubtitle',
                  })}
            ...
            return (
              ...
              >
                <PracticeCheckpointCard
                  ...
                  subtitle={intl.formatMessage({
                    id: 'prepositionsPracticeListScreen.finalTestSubtitle',
                  })}
                  status={progress.status}
                  isFinal={true}
                  onPress={() => handleLevelPress(lvl)}
          ...
          const verbsString = Array.isArray(lvl.verbs)
            ? lvl.verbs.map(v => `${v.infinitive} (${v.prep})`).join(', ')
            : '';
          ...
          return (
            <TouchableOpacity
              ...
              testID={`preposition-level-${lvl.id}`}
            >
