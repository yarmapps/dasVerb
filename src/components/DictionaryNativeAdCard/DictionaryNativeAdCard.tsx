...
import { isDictionaryNativeAdEnabled } from '../../services/appConfigService';
import { ThemeColors } from '../../styles/themeColors';
import { createStyles } from './DictionaryNativeAdCard.styles';

export { isDictionaryNativeAdEnabled };

export interface DictionaryNativeAdCardProps {
  isPremium: boolean;
  colors: ThemeColors;
  isDark?: boolean;
}

export function DictionaryNativeAdCard({
  isPremium,
  colors,
  isDark = false,
}: DictionaryNativeAdCardProps): React.JSX.Element | null {
  const intl = useIntl();
  ...
  const isFeatureEnabled = isDictionaryNativeAdEnabled();
    ...
    NativeAd.createForAdRequest(NATIVE_AD_UNIT_ID, {
      ...
      .catch(error => {
        if (__DEV__) {
          ...
          console.warn('[DictionaryNativeAdCard] Failed to load native ad:', error);
        }
        if (isMounted) {
          setHasError(true);
        }
  ...
  return (
    <View style={styles.cardWrapper} testID="dictionary-native-ad-card">
      <NativeAdView
        nativeAd={nativeAd}
